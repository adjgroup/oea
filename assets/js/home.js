...
function resizeMoniter(){
    ...
    contentImgScreen.querySelector("img").style.width=`${itemWidth}px`;
...
function appendList(){
    ...
    if( innerSize-scrollSize-outerSize < 100 ){
        ...
        nextBottomIndex++;
        if(nextBottomIndex==listData.length) nextBottomIndex=0;
    }
    listItems = contentInner.querySelectorAll("div");
}
...
function activeItem(){
    ...
    // 선택된 목록에 맞는 이미지 셋팅
    let imgIndex = listItems[currentIndex].querySelector("img").getAttribute("value");
    changeImg(imgIndex);
...
function changeImg(index){ 
    ...
    // 기존 이미지 제거
    let oldImg= contentImgScreen.querySelector("a:last-child");
    if(oldImg != undefined){
...
function init(){
    ...
    nextTopIndex = listData.length-1;
    nextBottomIndex = 0;
    blockFlag = false;
    ...
    // 초기 active 셋팅
    listItems = contentInner.querySelectorAll("div");
    currentIndex = Math.ceil(listItems.length/listData.length/2) * listData.length;
    activeItem();
